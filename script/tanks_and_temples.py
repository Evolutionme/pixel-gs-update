...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s tanks_and_temples/{scene} -m {output_dir}/{scene} --eval -r {factor} --port {6009+int(gpu)} --data_device cpu"
    print(cmd)
    if not dry_run:
        os.system(cmd)
